# 飞书 Bot 部署经验总结（WebSocket 模式）

> 资料来源：【AIGC】Hermes Agent 飞书 Bot 部署经验总结（博客园 Timmoc，2026-07-05）
> 原文链接：https://www.cnblogs.com/timmoc/p/21137621
>
> 本文档记录在 Ubuntu 24.04 虚拟机上从零部署 Hermes Agent（NousResearch/hermes-agent），接入智谱 bigmodel 作为 LLM 后端，并打通飞书 WebSocket 双向消息的完整流程与踩坑点。
...
Hermes 内置 `zai` provider 专门对接智谱。`provider: "custom"` 会**强制走本地推理服务器路径**，用 `no-key-required` placeholder，**忽略所有 API key**（含 `OPENAI_API_KEY`）。
...
3. **gateway 必须 Python Popen + start_new_session** —— `setsid` 在 SSH exec_command 里不够
4. **WebSocket 模式 > Webhook 模式** —— 不需要公网 IP/端口映射
5. **lark_oapi 必须手工装** —— hermes 默认 deps 没包含飞书 SDK
